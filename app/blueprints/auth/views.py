...
def update_user():
    if request.method == 'POST':
        ...
        db.session.commit()
        flash("User's information has been updated successfully", 'success')
...
@auth_bp.route('/update/about_me', methods=['POST'])
def update_user_about_me():
    if request.method == 'POST':
        user = User.query.get(current_user.id)
        form = request.form
        user.about_me = form['about_me']
        db.session.commit()
        flash("User's About Me has been updated successfully", 'success')
        return redirect(url_for('main.profile'))


@auth_bp.route('/update/about_me_again', methods=['GET','POST'])
def update_user_about_me_again():
    if request.method == 'POST':
        user = User.query.get(current_user.id)
        ...
        user.about_me = form['about_me_again']
        print(f'user is: {user}, form is: {form}, text is: {user.about_me}')
        db.session.commit()
        flash("User's About Me has been updated successfully", 'success')
        return redirect(url_for('main.profile'))
